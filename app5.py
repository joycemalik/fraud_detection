...
def monitor_audio(selected_language="en"):
    global monitoring, conversation_log
    while monitoring:
        audio, dur = record_chunk()
        chunk_txt = transcribe(audio, language=selected_language)
        if chunk_txt:
...
def start_monitoring():
    ...
    if not monitoring:
        ...
        selected_language = req.get("language", "en")

        thread = threading.Thread(target=monitor_audio, args=(selected_language,))
        thread.start()
        return jsonify({"message": "Monitoring started.", "language": selected_language})
